...
> 8. SenseNova-U1.5 step execution supports t2i (with and without think) and it2i, including step-wise batching with `--max-num-seqs > 1`; each wave runs one transformer forward per request (no packed forward). It cannot be combined with a diffusion cache backend; see [Diffusion Execution Modes](diffusion/execution_modes.md#step-execution).
