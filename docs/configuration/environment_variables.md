...
| `VLLM_OMNI_CHUNK_NOTIFY` | `1`, `true`, `yes` or `on` enables; default `0` (off) | `SharedMemoryConnector` chunk notifications (RFC #6870 task B1); read when each connector is created | A connector's `extra["chunk_notify"]` in the deploy yaml overrides the environment for that edge. Enables an IPC notification so the receiving stage sleeps until a chunk lands instead of polling on a fixed backoff; a lost notification falls back to polling, never losing data. | Experimental |
| `VLLM_OMNI_CHUNK_NOTIFY_FALLBACK_MS` | Positive float ms; default `25` | Chunk-transfer recv loop when notifications are enabled; read when the transfer adapter is constructed | Environment-only setting. Fallback re-poll interval recovering dropped notifications. A non-float or `<=0` logs a warning and uses the default. | Experimental |
| `VLLM_OMNI_CHUNK_NOTIFY_SALT` | String; default unset | `SharedMemoryConnector` notification endpoints; read per endpoint construction | Distinguishes notification sockets of concurrent deployments on one host. A collision only disables the hint (bind failure falls back to polling). | Experimental |
| `VLLM_OMNI_CHUNK_RECV_BACKOFF_MS` | Positive float ms; default `1` | Chunk-transfer recv loop poll cadence for connectors without notifications; read when the transfer adapter is constructed | Environment-only setting; the default matches the historical fixed backoff. A non-float or `<=0` logs a warning and uses the default. | Stable operational control |
