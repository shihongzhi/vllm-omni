...
class ErnieImageTransformer2DModel(nn.Module):
    # Cache-DiT is enabled through the model-specific enabler
    # ``enable_cache_for_ernie_image``: has_separate_cfg must match the
    # per-step forward count, which differs between CFG (base) and
    # distilled (Turbo) checkpoints.
